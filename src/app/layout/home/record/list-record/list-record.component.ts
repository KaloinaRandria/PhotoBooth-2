...
export class ListRecordComponent implements OnInit{
  ...
  delete(record:any){
    ...
    this.recordService.delete(url).subscribe(
      (response:any) =>{
        if(response.success){
          const index = this.records.findIndex(ro => ro.id_categorie_theme === record.idDepense);
  ...
  filterRecord(
    ...
    return recordList.filter(record => {
      ...
      const matcheslibele = filter.label && filter.label !== '' ? record.libele.includes(filter.label) : true;
      ...
      if (filter.startDate && filter.startDate !== '') {
        matchesStartDate = new Date(record.date_insertion) >= new Date(filter.minDate);
      }

      if (filter.endDate && filter.endDate !== '') {
        matchesEndDate = new Date(record.date_insertion) <= new Date(filter.maxDate);
      }
      return (
        matcheslibele &&
        matchesCoutMin &&
